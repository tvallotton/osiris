...
/// spawning a new task on drop of the returned future, or by leaking the
/// future, if the task couldn't be spawned.
///
/// # Example
/// ```rust
/// # use std::task::Poll;
...
impl<F: Future + 'static + Unpin> Drop for Completion<F> {
    fn drop(&mut self) {
        if let Some(future) = self.future.take() {
            let future = future;
            let mut future = complete(future);
            if let Some(rt) = current() {
                rt.spawn(poll_fn(move |cx| Pin::new(&mut future).poll(cx)));
            } else {
                forget(future);
                panic!("dropped a completion future outside a runtime context.")
            }
        }
    }
}
